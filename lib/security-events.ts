...
export async function recordSecurityEvent(input: { source: string; type: string; ip?: string | null; message: string; isBot?: boolean | null; botName?: string | null; metadata?: Record<string, string | number | boolean | null> }) {
  try {
    ...
    await prisma.securityEvent.create({
      data: {
        ...
        isBot: input.isBot ?? null, botName: input.botName?.slice(0, 120) || null,
